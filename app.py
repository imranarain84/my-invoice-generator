...
def extract_backmarket_data(uploaded_file):
    ...
    # 5. Table Extraction (Looping for multiple items)
    grand_total = "£0.00"
    if tables:
        for row in tables[0]:
            # Back Market Item rows typically start with a number (Line No)
            ...
            # Capture the final total if present in table footer
...
def create_invoice_pdf(data):
    ...
    # INVOICE BAR (Added Shipping Method)
    ...
    # TABLE BODY (Looping through all extracted items)
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Arial", size=9)
    ...
    msg_parts = [
        ("Hi ", 'normal'), (data['first_name'], 'bold'), (",", 'normal'),
        (f"\n\nWe hope you enjoy your order #{data['order_no']} from Vertical Passage LTD.", 'normal'),
        ("\nLooks like your phone just found its new favorite case.", 'normal'),
        ('\n\nNeed help? Just log in to your Back Market account, go to Orders, and click "Get Help."', 'normal'),
        ("\n\nEnjoy your new case,\n", 'normal'), ("Vertical Passage", 'bold')
    ]
    
    for text, style in msg_parts:
        pdf.set_font("Arial", 'B' if style == 'bold' else '', 10)
        # Simplified centering for the message block
...
if f:
    ...
    st.success(f"Order {data['order_no']} for {data['first_name']} loaded.")
